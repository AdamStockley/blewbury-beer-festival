# Drink Artwork Status

## Added from official producer sources

- Loddon Brewery — Hullabaloo pump clip
- Loddon Brewery — Dragon Slayer pump clip
- Bond Brews — Bengal Tiger pump clip
- Bond Brews — Railway Porter pump clip
- Tutt's Clump Cider — Reading Gold product image
- Tutt's Clump Cider — Rum Cask product image

## Still to source

- Mysterious Brewing — Jürgen
- Mysterious Brewing — Aurum
- Little Ox Brewery — Dark & Seedy
- Little Ox Brewery — Daydreamer
- Tap Social — Embers
- Sharp's Brewery — Twin Coast
- Brains Brewery — SA

The cards no longer use initial letters. Until suitable official artwork is added, they display a quiet "Artwork coming soon" treatment.

When wine and gin products are confirmed, use the same rule:
1. official product image / label / pump clip
2. official producer logo
3. neutral artwork placeholder
